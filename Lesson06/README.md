[System Integration - Spring 2026](https://github.com/arturomorarioja-ek/SD_System_Integration_F2026/blob/main/README.md)

# Lesson 6 - 12 March

### In-class exercises

- Practice GraphQL in the following playgrounds:
  - [Moonhighway's Trail Events playground](https://snowtooth.moonhighway.com/)
  - [Moonhighway's Pet Library playground](https://pet-library.moonhighway.com/) 
  - [Hasura's Photo Albums playground](https://cloud.hasura.io/public/graphiql?endpoint=https%3A%2F%2Fgraphqlzero.almansi.me%2Fapi)
  - [Star Wars playground](https://graphql.org/swapi-graphql/)
  - [Users playground](https://graphqlplaceholder.vercel.app/graphql)
  - [Photo Albums playground](https://graphqlzero.almansi.me/api)
  - [Company playground](https://www.devtoolsdaily.com/graphql/playground/)
  - [Hypgraph's Movies playground](https://hygraph.com/graphql-playground)
- GraphQL SDL: [Movie Streaming](https://github.com/arturomorarioja-ek/SD_System_Integration_F2026/blob/main/Lesson06/02%20Movie%20Streaming.md)

### Homework

- Check out the **GraphQL** slides
- Check out the following code samples:
  - GraphQL API consumption: [Star Wars](https://github.com/arturomorarioja/js_graphql_star_wars)
  - GraphQL server implementation: [Books (Python/Grafene)](https://github.com/arturomorarioja/py_books_graphql)
  - GraphQL API and Apollo Server implementation: [Books (JavaScript/Apollo Server)](https://github.com/arturomorarioja/js_books_graphql)
- Solve the following exercises:
  - GraphQL practice: [Countries](https://github.com/arturomorarioja-ek/SD_System_Integration_F2026/blob/main/Lesson06/01%20Countries.md)
  - GraphQL practice: solve Part II of [the Movie Streaming exercise](https://github.com/arturomorarioja-ek/SD_System_Integration_F2026/blob/main/Lesson06/02%20Movie%20Streaming.md)
  - GraphQL API: [Books](https://github.com/arturomorarioja-ek/SD_System_Integration_F2026/blob/main/Lesson06/03%20%20Books%20API.md)
  - GraphQL API: [Tasks](https://github.com/arturomorarioja-ek/SD_System_Integration_F2026/blob/main/Lesson06/04%20Tasks%20API.md)
